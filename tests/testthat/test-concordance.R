...
test_that("issue #25: the dose reaches the right compartment when $MODEL and d/dt order differ", {
  ...
  # starting values can rescue that. Measured on ferx 0.3.0: 0.000000 at every
  # observation time before the fix, and the reference profile after it.
  #
  # THE DATASET IS SIMULATED BY NONMEM, not by us, and that is the point of it.
  # Every other dataset in inst/testdata/ is produced by translating the model
  # and simulating the RESULT, so whatever the translator decided is baked into
  # both sides and cancels -- those tests check internal consistency, not
  # fidelity to NONMEM, and cannot fail for a whole class of defect (see #32,
  # where a wrong obs_scale survived the entire Tier-4 suite untouched).
  #
  # This one is generated by running the .ctl through NONMEM 7.6.0 and taking its
  # simulated DV, so a translation that disagrees with NONMEM shows up here as a
  # failed recovery. Regenerate it the same way -- see
  # data-raw/generate_concordance_data.R and tests/nonmem-anchor/README.md -- and
  # do NOT regenerate it with ferx_simulate(), which would silently turn it back
  # into a self-consistency check.
