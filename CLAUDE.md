...
The `engine` job pins `ferx` (currently `ferx-r@731adc9` = 0.2.0). The reference
omegas in `test-concordance.R` AND the bundled datasets in `inst/testdata/`
(regenerated by `data-raw/generate_concordance_data.R` against that engine) are
tied to that build - bump the pin and re-baseline both in the same commit.
