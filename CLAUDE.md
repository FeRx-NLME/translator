...
`tools/update-ferx-core-lock.sh`), then this pin to that ferx-r commit. Use that
script rather than cargo directly: ferx-r's `src/rust/.cargo/config.toml` carries
a `[patch]` redirecting ferx-core to a sibling `../ferx-core` checkout when one is
present, and any cargo command that rewrites the lock while it is active writes a
path-style entry with no `source = "git+..."` line - silently unpinning ferx-core
for everyone who builds without the sibling, CI included. It has happened twice
upstream (ferx-r `1ce7f59`, `b96c867`).
