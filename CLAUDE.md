...
The `engine` job pins `ferx` (currently `ferx-r@7889719` = 0.3.0, tag `v0.3.0`).
Pin the SHA, not the tag: a tag can be moved, and this job is the only thing
between a silent engine regression and a merge.

Keep the pin on the version users actually install. It sat on `ferx-r@731adc9`
(0.2.0) after 0.3.0 shipped, which meant every green engine job proved only that a
build nobody runs accepted the output.
